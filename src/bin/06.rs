...
impl Guard {
    ...
    fn do_move(&mut self, map: &Matrix<char>) -> &GuardState {
        ...
        if new_pos.0 >= 0
            && new_pos.0 < map.columns as isize
            && new_pos.1 >= 0
            && new_pos.1 < map.rows as isize
        {
            ...
            if self
                .visited
                .contains(&(self.x, self.y, self.direction.clone()))
            {
                self.state = GuardState::Looping;
            }

            self.visited
                .insert((self.x, self.y, self.direction.clone()));
        } else {
            let fields_visited: HashSet<(usize, usize)> =
...
impl GuardSimulation {
    ...
    fn step(&mut self) {
        ...
        match new_guard_state {
            ...
            GuardState::Looping => self.state = SimState::GuardLooped,
        }
    }
}
...
pub fn part_two(input: &str) -> Option<u32> {
    ...
    for (x, y, d) in &guard_walk {
        ...
        match d {
            ...
            Direction::Left => obstacle = ((*x as isize - 1), *y as isize),
        }
        if obstacle.0 < 0
            || obstacle.0 >= matrix.columns as isize
            || obstacle.1 < 0
            || obstacle.1 >= matrix.rows as isize
        {
            continue;
        } else {
